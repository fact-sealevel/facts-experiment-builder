...
def format_compose_yaml(content: str) -> str:
    ...
    prev_line_was_depends_on_service = (
        False  # Track if previous line was a service name under depends_on
    )
    ...
    for i, line in enumerate(lines):
        ...
        if stripped == "depends_on:":
            ...
            and stripped.endswith(":")
            and not is_list_item
            and not stripped.startswith("condition")
            and stripped != "depends_on:"
        ):
            ...
            not in_service
            and leading_spaces <= 6
            and stripped.endswith(":")
            and stripped not in ["depends_on:", "fair:", "condition:"]
        ):
        ...
        if i > 0 and prev_line_was_list_property and is_list_item:
            ...
            if in_service:
                ...
                formatted_lines.append("               " + stripped)
            prev_line_was_depends_on_service = False
            in_depends_on_service = (
                True  # Keep tracking for potential future properties
            )
        elif in_depends_on_service and stripped.startswith("condition"):
            ...
            prev_line_was_list_property = stripped.endswith(":") and stripped in [
                "command:",
                "volumes:",
                "depends_on:",
            ]
