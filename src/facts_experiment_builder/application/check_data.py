...
def plan_input_checks(
    ...
    Args:
        inp: The input arg-spec entry.
        output_volume_input_keys: This module's input `name`s that mount from the
            shared output volume (from `ModuleSchema.get_output_volume_input_keys()`),
            i.e. files generated by another module at runtime rather than provided
            by the user.
    """
    field_name = inp.get("name", "")
    if field_name in output_volume_input_keys:
        return [
            PlannedCheck(
                field_name,
                None,
    ...
    if inp.get("mount", {}).get("container_path") == _SHARED_CONTAINER_PATH:
        return []
...
def _check_module(
    ...
    plans = [
        ...
        for plan in plan_input_checks(inp, output_volume_input_keys)
    ]
...
def check_shared_data(
    ...
    for module_name in discovered_module_names:
        ...
        output_volume_input_keys = schema.get_output_volume_input_keys()
        for inp in schema.arguments.get("inputs", []):
            field_name = inp.get("name", "")
            filename = inp.get("filename")
            container_path = inp.get("mount", {}).get("container_path", "")
            if field_name in output_volume_input_keys or not filename:
                continue
