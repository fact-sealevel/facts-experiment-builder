...
class ModuleServiceSpec:
    ...
    def _resolve_value(self, source: str) -> Any:
        ...
        context = {"metadata": self.components.metadata, "module_inputs": self.components}
    ...
    def _build_depends_on(self, temperature_service_name: Optional[str] = None) -> Dict[str, Any]:
        """
        ...
        if uses_climate_file and temperature_service_name:
            depends_on[temperature_service_name] = {"condition": "service_completed_successfully"}
        ...
        if depends_on_config:
            for dep_spec in depends_on_config:
                if isinstance(dep_spec, dict):
                    ...
                    depends_on[mapped_name] = {'condition': 'service_completed_successfully'}
    ...
    def generate_compose_service(self, temperature_service_name: Optional[str] = None) -> Dict[str, Any]:
        ...
        """
        image_str = f"{self.components.image.image_url}:{self.components.image.image_tag}"
    ...
    def generate_asyncflow_config(self) -> Dict[str, Any]:
        ...
        """
        raise NotImplementedError("AsyncFlow configuration generation is not implemented")
