...
def test_setup_experiment_runs_successfully(
    ...
    experiment_metadata_path = Path(
        project_root / experiment_name / "experiment-config.yaml"
    )
    experiment_dir = experiment_metadata_path.parent
    experiment_dir.parent.mkdir()
    # experiment_dir.mkdir(parents=True)

    print("Experiment metadata path!!!: ", experiment_metadata_path)
    ...
    # assert (project_root / experiment_name / "experiment-config.yaml").exists(), (
    #    f"Expected experiment-config.yaml written at {project_root}/{str(experiment_name)}/experiment-config.yaml"
    #    " ---- Output ----"
    #    f"{result.output}"
    #    " ---- Exception ----"
    #    f"{result.exception}"
    ...
    all_module_names = [
        spec.module_name for step in experiment_steps for spec in step.module_specs()
    ]
