...
def test_module_requires_climate_file_false_when_key_false(tmp_path: Path):
    ...
    with (
        patch("facts_experiment_builder.application.generate_compose.find_project_root", return_value=tmp_path),
        patch("facts_experiment_builder.application.generate_compose.find_module_yaml_path", return_value=module_yaml),
    ):
